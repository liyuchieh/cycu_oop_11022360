...
def plot_bus_route_with_marker(csv_data, geojson_path, output_image="Bus_Route_Map.jpg", station_number=None, marker_image_path=None):
    """
    根據 CSV 資料繪製公車路線圖，並在指定車站號碼的位置放置圖片標記。
    
    :param csv_data: 包含 latitude 和 longitude 的 DataFrame
    :param geojson_path: GeoJSON 檔案路徑
    :param output_image: 輸出的地圖圖片檔案名稱
    :param station_number: 指定的車站號碼
    ...
    try:
        ...
        # 如果指定了車站號碼和標記圖片
        if station_number is not None and marker_image_path is not None:
            # 找到對應的車站資料
            station_data = csv_data[csv_data['車站序號'] == station_number]
            if not station_data.empty:
                station_lat = station_data['latitude'].values[0]
                station_lon = station_data['longitude'].values[0]
                
                # 加載標記圖片
                img = plt.imread(marker_image_path)
                imagebox = OffsetImage(img, zoom=0.03)  # 調整圖片大小
                ab = AnnotationBbox(imagebox, (station_lon, station_lat), frameon=False, zorder=10)
                ax.add_artist(ab)
...
if __name__ == "__main__":
    try:
        ...
        if csv_data is not None:
            ...
            # 使用者輸入車站號碼
            station_number = input("請輸入車站號碼 (例如：1): ").strip()
            station_number = int(station_number) if station_number.isdigit() else None
            
            # 指定標記圖片的路徑
            marker_image_path = "C:\\Users\\User\\Desktop\\cycu_oop_11022360\\20250429\\人.png"
            
            # 繪製公車路線圖並添加標記
            output_image = f"data/ebus_taipei_{routeid}_{direction}_map.jpg"
            plot_bus_route_with_marker(csv_data, geojson_path, output_image, station_number, marker_image_path)
    except Exception as e:
        print(f"發生錯誤：{e}")